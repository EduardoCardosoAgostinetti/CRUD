...
router.post('/create', express.json(), (req, res) => {
  ...
  mysql.query('INSERT INTO users (name, email) VALUES (?, ?)', [name, email], (err, results) => {
    if (err) {
      res.status(500).json({ error: err });
    } else {
...
router.put('/update/:id',  express.json(), (req, res) => {
  const { id } = req.params;
  const { name, email } = req.body;
  mysql.query('UPDATE users SET name = ?, email = ? WHERE iduser = ?', [name, email, id], (err) => {
    if (err) {
      res.status(500).json({ error: err });
    } else {
...
router.delete('/delete/:id', (req, res) => {
  const { id } = req.params;
  mysql.query('DELETE FROM users WHERE iduser = ?', [id], (err) => {
    if (err) {
      res.status(500).json({ error: err });
    } else {
